...
INITIAL_STOP_ATR = [1.0, 1.25, 1.5, 2.0]
TRAIL_ATR = [1.0, 1.5, 2.0, 2.5, 3.0]
HOLDING_BARS = {"1h": [24, 48, 120], "4h": [12, 24, 60], "1d": [5, 10, 15]}


def load_data():
    if not DATA_FILE.exists():
        raise FileNotFoundError(f"Missing {DATA_FILE}. Run the downloader first.")
...
def resample(df, rule):
    return df.resample(rule, origin="epoch").agg({"open":"first", "high":"max", "low":"min", "close":"last", "volume":"sum"}).dropna()


def add_indicators(df, n):
    x = df.copy()
    ...
    pdi, mdi = 100*plus/x.atr14, 100*minus/x.atr14
    x["adx14"] = (100*(pdi-mdi).abs()/(pdi+mdi)).rolling(14).mean()
    change = x.close.diff()
    ...
    x["candle_position"] = (x.close-x.low)/(x.high-x.low).replace(0, np.nan)
    return x.dropna()


def signal_masks(x, adx_min):
    long_mask = ((x.close > x.prior_high*1.0015) & (x.ema20 > x.ema50) & (x.adx14 >= adx_min) & (x.rsi14 >= 50) & (x.candle_position >= .65))
    short_mask = ((x.close < x.prior_low*.9985) & (x.ema20 < x.ema50) & (x.adx14 >= adx_min) & (x.rsi14 <= 50) & (x.candle_position <= .35))
    return long_mask, short_mask


def trade(x, i, side, target, hold, initial_atr_mult, trail_atr_mult):
    entry_i = i + 1
    if entry_i >= len(x):
        return None
    entry = float(x.iloc[entry_i].open)
    signal_atr = float(x.iloc[i].atr14)
    ...
    stop = entry-initial_atr_mult*signal_atr if side == "long" else entry+initial_atr_mult*signal_atr
    best = entry
    target_hit = False
    target_bars = np.nan
    exit_price = None
    ...
    return {"signal_date": x.index[i], "entry_date": x.index[entry_i], "exit_date": path.index[exit_i], "side": side, "entry": entry, "exit": exit_price, "target": target, "target_hit": target_hit, "target_bars": target_bars, "exit_reason": exit_reason, "gross_return": gross, "net_return": gross-COST, "mfe": mfe, "initial_stop_atr": initial_atr_mult, "trail_atr": trail_atr_mult, "hold_bars": hold}


def run(frame, timeframe):
    all_rows = []
    for n in DONCHIAN_LENGTHS:
        x = add_indicators(frame, n)
        for adx_min in ADX_THRESHOLDS:
            long_mask, short_mask = signal_masks(x, adx_min)
            for side, mask in [("long", long_mask), ("short", short_mask)]:
                for target in TARGETS:
                    for hold in HOLDING_BARS[timeframe]:
                        for initial_stop in INITIAL_STOP_ATR:
                            for trail in TRAIL_ATR:
                                for i in np.flatnonzero(mask.to_numpy()):
                                    result = trade(x, int(i), side, target, hold, initial_stop, trail)
                                    if result:
...
def summarize(events):
    ...
    out = grouped.agg(trades=("net_return", "size"), target_rate=("target_hit", "mean"), avg_net_return=("net_return", "mean"), median_net_return=("net_return", "median"), average_mfe=("mfe", "mean"), winners=("net_return", lambda x: int((x > 0).sum())), losers=("net_return", lambda x: int((x <= 0).sum())), gross_profit=("net_return", lambda x: x[x > 0].sum()), gross_loss=("net_return", lambda x: abs(x[x < 0].sum()))).reset_index()
...
def main():
    raw = load_data()
    frames = {"1h": raw, "4h": resample(raw, "4h"), "1d": resample(raw, "1D")}
